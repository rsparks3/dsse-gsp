% This code applies the GSP technique for a 3 phase unbalanced case (IEEE 34 bus test case)
% We assume that sensors are placed everywhere, similar to results
% generated for the single phase 85 bus test case
clc;
clear all;
close all;
...
% Assume sensors at all locations 
bus_name_sensor = unique_bus_names;
...
cvx_begin 
    ...
    variable v_est(nb)
    variable a_est(nb)
    minimize (35*norm(sm - M*s_est, 2) + 5*norm(vm.*vm-M*(v_est),2) + 5*norm(cm.*cm-M*a_est,2) )
...
% csvwrite('phase_a_mag.csv',[transpose(1:length(phase_a_actual)) phase_a_actual phase_a_vcon])
% csvwrite('phase_b_mag.csv',[transpose(1:length(phase_b_actual)) phase_b_actual phase_b_vcon])
% csvwrite('phase_c_mag.csv',[transpose(1:length(phase_c_actual)) phase_c_actual phase_c_vcon])
% 
% 
% csvwrite('phase_a_angle.csv',[transpose(1:length(phase_a_actual_angle)) phase_a_actual_angle phase_a_vcon_angle])
% csvwrite('phase_b_angle.csv',[transpose(1:length(phase_b_actual_angle)) phase_b_actual_angle phase_b_vcon_angle])
% csvwrite('phase_c_angle.csv',[transpose(1:length(phase_c_actual_angle)) phase_c_actual_angle phase_c_vcon_angle])
