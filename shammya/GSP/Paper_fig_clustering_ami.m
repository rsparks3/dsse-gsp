...
base_case = mpc_case;
data = load('85bus_scenario_only_load.mat');
PD_case = data.PD_case;
QD_case = data.QD_case;
...
for i = 1:ncluster
    ...
    SENSOR_PER_CLUSTER_DATA = min([length(nodes),SENSOR_PER_CLUSTER]);
    [~,sl,al]= greedy_placement(U_k,SENSOR_PER_CLUSTER_DATA,[],nodes',G,f,t,4,Yuk(:,1:number_of_eigenvectors),Yik(:,1:number_of_eigenvectors));
...
save(strcat(casestr,'_clustering_scenario_ami.mat'),'failed_scenario','estimated_angle','actual_angle','estimated_voltage','actual_voltage','sensor_locations')
