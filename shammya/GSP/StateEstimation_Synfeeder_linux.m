...
for i = 1:ncluster
    ...
    [~,sl,al]= greedy_placement(U_Y_new(:,1:n_eigs),sensor_per_cluster,[],nodes',G,f,t,4,Yuk(:,1:n_eigs),Yik(:,1:n_eigs));
...
fprintf('Sensor Placement Algorithm Completed. \n')
...
fprintf('Necessary Multiplications completed. \n')
...
cvx_end
fprintf('CVX_Status with GSP:%s',cvx_status);
...
cvx_begin quiet
    ...
    subject to
        ...
        sum(imag(s_est)) <=0.1;
cvx_end
fprintf('CSV_STATUS without GSP:%s\n',cvx_status);
if strcmpi('Solved',cvx_status) || strcmpi('Inaccurate/Solved',cvx_status)
