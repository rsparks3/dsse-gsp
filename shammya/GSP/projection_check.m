...
save(strcat(casestr,'_covariance.mat'),'covariance');
...
sensor_locations = 1:number_of_sensors;
sensor_locations(1)= [];
% sensor_locations = [1 2 4 5 6  7 8 9 11 13 14 16 17 19 20 21 ];
...
% cvx_solver mosek
% cvx_begin 
%     variable W(number_of_eigenvectors,number_of_eigenvectors) complex semidefinite
%     minimize (norm(sm - M*conj(diag(Y_DD*U_k*W*U_k')), 2) + norm(vm.*vm-diag(M*U_k*W*(U_k)'*M'),2) ) ; 
%     subject to
%         W == W';
% %         W >= 0 ;
% cvx_end
...
% 
% [U,S,V] = svd(W);
% v_tilde_1 = sqrt(S(1,1)) * U(:,1);
% v_recon_1 = U_k*v_tilde_1;
% 
% % v_tilde_2 = sqrt(S(1,1)) * V(:,1);
% % v_recon_2 = U_k*v_tilde_2;
% 
% norm((abs(vcomplex)-abs(v_recon_1)),2)
% 
% 
% % v_recon_3 = U_k*diag(sqrt(abs(W)));
% 
% figure 
% plot(abs(vcomplex))
% hold on 
...
% legend('true','recon1')
% xlabel('Bus Numbers')
% title('Reconstruction with Direct W')
% 
% 
% figure 
% plot(angle(vcomplex))
...
cvx_begin 
    variable tildeW(number_of_eigenvectors,number_of_eigenvectors) complex semidefinite
     minimize (norm(sm - M*conj(diag(Y_DD*U_k*tildeW*U_k')), 2) + norm(vm.*vm-diag(M*U_k*tildeW*(U_k)'*M'),2))  
