...
missing_sensor = 5;
TotalScenario = combnk(1:nb, missing_sensor);
...
for scenario = 1: 1:100
    sensor_locations = 1:nb;
    sensor_locations(TotalScenario(scenario,:))= [];
    ...
    cvx_begin 
     ...
     cvx_end
     W = U_k * tildeW * U_k';
     voltage_dif_scenario = max(abs(abs(v) - sqrt(abs(diag(W)))));
     ...
     angle_dif_scenario = max(abs(vangle - voltage_angle));
     if voltage_diff > voltage_dif_scenario && angle_diff > angle_dif_scenario
         angle_diff = angle_dif_scenario;
         voltage_diff = voltage_dif_scenario;
         saved_scenario = TotalScenario(scenario,:);
     end
