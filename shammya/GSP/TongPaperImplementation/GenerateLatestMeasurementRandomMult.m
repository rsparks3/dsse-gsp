...
P_D = mpc.bus(:,PD);
Q_D = mpc.bus(:,QD);
input_data_2020 = zeros(40000,84*2+length(ammeter_location));
output_data_2020 = zeros(40000,85*2);
...
%         mult = demand(s)/max(demand);
        mult = 0.9 + (1.1-0.9).*rand(1,1);
%         if mod(s,100) == 0
%             mult = 0.99 + (1.01-0.99).*rand(1,1);
%         end
        if s == 40000
            mult = 1;
        end
...
save('input_data_randmult.mat','input_data_2020');
save('output_data_randmult.mat','output_data_2020');
