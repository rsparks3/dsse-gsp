...
[N,~]=size(U_k);
i=1;
j_tilde = 1:length(unique_bus_names);
% j_tilde(M_s)=[];
while i <= M_tilde
    sigma_min_uk=zeros(length(unique_bus_names),1);
    sigma_min_yuk=zeros(length(unique_bus_names),1);
    sigma_min_uk(M_s) = nan; sigma_min_yuk(M_s) = nan;
    for j=1: length(unique_bus_names)
        if ~any(M_s == j) 
            sensor_bus_map = bus_name_loc(unique_bus_names{j});
...
%         [~,min_ind]=max(abs(sigma_min_uk)+abs(sigma_min_yuk));
    [~,min_ind]= max(min([sigma_min_uk sigma_min_yuk],[],2));
    M_s = [M_s;j_tilde(min_ind)];
    sensor_placed_map = bus_name_loc(unique_bus_names{M_s(end)});
...
bus_name_sensor = unique_bus_names(M_s);
...
sensor_locations = M_s_y;
total_sensors = length(M_s);
...
csvwrite('phase_a_mag_sensor.csv',[transpose(1:length(phase_a_actual)) phase_a_actual phase_a_vcon])
csvwrite('phase_b_mag_sensor.csv',[transpose(1:length(phase_b_actual)) phase_b_actual phase_b_vcon])
csvwrite('phase_c_mag_sensor.csv',[transpose(1:length(phase_c_actual)) phase_c_actual phase_c_vcon])


csvwrite('phase_a_angle_sensor.csv',[transpose(1:length(phase_a_actual_angle)) phase_a_actual_angle phase_a_vcon_angle])
csvwrite('phase_b_angle_sensor.csv',[transpose(1:length(phase_b_actual_angle)) phase_b_actual_angle phase_b_vcon_angle])
csvwrite('phase_c_angle_sensor.csv',[transpose(1:length(phase_c_actual_angle)) phase_c_actual_angle phase_c_vcon_angle])
